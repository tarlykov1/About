...
base_calls AS (
    SELECT
        ...
        COALESCE(ts_scd.id, ts_sc.id)                                   AS service_id,
        COALESCE(ts_scd.title, ts_sc.title)                             AS service_title,
        COALESCE(ts_scd.inventory_Number, ts_sc.inventory_Number)       AS service_code,
        COALESCE(ts_scd.calculateCost, ts_sc.calculateCost)             AS service_formula_id
    FROM tbl_servicecall sc
    LEFT JOIN tbl_servicec$accessca_providea scd
        ON sc.id = scd.servicecall_id
    LEFT JOIN tbl_slmservice ts_scd
        ON ts_scd.id = scd.accessca$providea
